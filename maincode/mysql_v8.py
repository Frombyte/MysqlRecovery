...
class MysqlV8:
    ...
    def get_lob_data(self,col,indx_list):
        for i in range(len(indx_list)):
            if i == 0:
                page_file.seek(indx_list[i][0] * 16384)
                data = page_file.read(16384)[0x2B8:0x2B8+indx_list[i][1]]
            else:
                page_file.seek(indx_list[i][0] * 16384)
                data = page_file.read(16384)[0x31:0x31+indx_list[i][1]]
            col += data
        return col
    ...
    def get_lob(self,file_no, page_no):
        col = bytearray()
        indx_list = []
        page_file.seek(16384 * page_no)
        f_indx_page = page_file.read(16384)
        indx_num = int.from_bytes(f_indx_page[0x40:0x44],'big')
        indx_page_no = int.from_bytes(f_indx_page[0x44:0x48],'big')
        indx_off = int.from_bytes(f_indx_page[0x48:0x4A],'big')
        while indx_page_no != 0xffffffff and indx_page_no != 0:
            page_file.seek(16384 * indx_page_no)
            page = page_file.read(16384)
            indx = page[indx_off:indx_off+0x3c]
            indx_page_no = int.from_bytes(indx[0x06:0x0a],'big')
            indx_off = int.from_bytes(indx[0x0a:0x0c],'big')
            data_page_num = int.from_bytes(indx[0x30:0x34],'big')
            data_size = int.from_bytes(indx[0x34:0x36],'big')
            indx_list.append([data_page_num,data_size])
        data = self.get_lob_data(col,indx_list)
        return data
    ...
    def get_varchar(self,col_data, type_com, lenth_bin, comments):
        ...
        try:
            col_tmp = col_data.decode(encoding='utf-8').rstrip().replace('\\', '\\\\').replace("'", "\\'")
            col_tmp = col_tmp.replace('\r', '\\r').replace('\n', '\\n')
            s = re.compile('[\\x00-\\x08\\x0b-\\x0c\\x0e-\\x1f]').search(col_tmp)
            if s is None:
                col = f"'{col_tmp}'"
            else:
                col = False
    ...
    def get_smallint(self,col_data, type_com, lenth_bin, comments):
        if unsigned == 0:
            ...
            col = f"'{col}'"
        return col
    def get_year(self,col_data, type_com, lenth_bin, comments):
        if len(col_data) == 1:
            col = str(struct.unpack('B', col_data)[0] + 1900)
            col = f"'{col}'"
            return col

    def get_mediumint(col_data, type_com, lenth_bin, comments):
        if unsigned == 0:
    ...
    def get_datetime(self,col_data, type_com, lenth_bin, comments):
        ...
        if col > '1970-01-01 00:00:00' and col < time_limit_str:
            if len(col_data) > 5:
    ...
    def process(self,struct_str, row_dir, page_file_dir, is_del,table_name_list,db_path):
        ...
        for table_name in list_all_name:
            ...
            if os.path.exists(page_file_path):
                ...
                for i in range(page_file_size // 16384):
                    if break_flag:
                        break
                    ...
                    if page_type != 0 and page_no > 3 and page_level == 0 and magic == b'supremum' :
                            ...
                            if f_record_off != 0:
                                ...
                                while next_record_off != 0:
                                    ...
                                    try:
                                        ...
                                        if f_record_off != 112:
                                            try:
                                                record_top = struct.unpack('B', page[f_record_off - 5:f_record_off - 4])[0] >> 7
                                            except:
                                                break
                                            if record_top == 0:
                                                record_top_size = 5
                                            ...
                                            if len(primary_key) == 0:
                                                ...
                                                for primary in primary_key:
                                                    ...
                                                    n += lenth
                                                    row_dic[col_id] = col
                                                    # print(page_no,f_record_off,col)
                                            end_lab = True
                                                        ...
                                                        if is_null_able == 1:
                                                            ...
                                                            if is_null == 0:
                                                                if lenth == 0:
                                                                    ...
                                                                    if lenth == 0:
                                                                        ...
                                                                        col_data = page[
                                                                                   f_record_off + n + 13:f_record_off + n + lenth + 13]
                                                                        col = getattr(self, type)(col_data, type, lenth_bin,
                                                                                                  comments)
                                                                    n += lenth
                                                                else:
                                                                    col_data = page[
                                                            ...
                                                            if lenth == 0:
                                                                ...
                                                                if lenth_temp >> 7 != 1:
                                                                    ...
                                                                    lenth_temp1 = struct.unpack('B', lenth_bin1)[0]
                                                                    lenth = ((lenth_temp & 0x7f) << 8) + lenth_temp1
                                                                    if type in ['get_blob', 'get_text', 'get_mediumtext',
                                                                                'get_longtext', 'get_longblob']:
                                                                        lenth = ((lenth_temp & 0x3f) << 8) + lenth_temp1
                                                                    x_no += 2
                                                                col_data = page[
                                                                           f_record_off + n + 13:f_record_off + n + lenth + 13]
                                            ...
                                            if row_dic != {}:
                                                ...
                                                str_value = f'insert into {table_name} values({str_value});\n'
